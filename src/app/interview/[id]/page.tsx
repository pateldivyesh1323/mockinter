...
export default function InterviewPage({ params }: InterviewPageProps) {
    ...
    const [interviewDetails, setInterviewDetails] = useState<InterviewDataType>();
    ...
    const fetchIntervieweeData = async (data: InterviewDataType) => {
        try {
            const res = await fetch(`/api/user/profile?searchid=${data.interviewee}`);
    ...
    const handleBooking = async () => {
        try {
            setLoading(true);
            const res = await fetch(`/api/interviews/book/${interviewDetails?._id}`, { method: "POST", });
            const data = await res.json();
            if (data.success) {
                console.log(data);
                ...
                <section className="w-[80%] m-auto">
                    ...
                    <div className='bg-white p-4 relative'>
                        ...
                        <div className='text-lg font-bold mb-4'>
                            Status: {interviewDetails?.status?.toUpperCase()} <br />
                            Price: {interviewDetails?.price === 0 ? "Free" : `${interviewDetails?.price}$`} <br />
                            {interviewee &&
                                <Link href={`/profile/${interviewee._id}`}>Interviewee: {interviewee.name}</Link>
                            }
                        </div>
                        {interviewDetails?.status === "upcoming" ?
                            <LoadingButton variant="contained" className="bg-black" sx={{ "&:hover": { backgroundColor: "#424242" } }} onClick={handleBooking} loading={loading}>Book Now</LoadingButton>
                            :
                            <Button variant="contained" disabled className="bg-black">{interviewDetails?.status}</Button>
                        }
                    </div>
                </section>}
        </>
